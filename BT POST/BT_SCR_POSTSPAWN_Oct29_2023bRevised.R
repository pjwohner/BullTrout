...
JAGSdata<-list(y=y, first=first, M=M, T=T, nreach=nreach,tempB=tempB,
               reach.loc=reach.loc,tempE=tempE, last=last
) 
...
parameters <- c("alphaphi","alphasig","betasig2",#"betaphiT",
                "betaT2","betaT3","tauv",
                "S","phi","z","sigma","lam0")
#Call JAGS
Sys.time()
...
Sys.time()
###################################################################################
save(ZZ,file="ZZ_1992_1994.Rdata")##save as robject

#check convergence in trace plots
MCMCtrace(ZZ, params = c("alphaphi","betaphiT","alphasig","betasig2",
                         "betaT2","betaT3","tauv"))
#get the draws
mco2 <- mcmcOutput(ZZ, params=c("alphaphi","betaphiT","alphasig","betasig2",
                                "betaT2","betaT3","tauv",
...
######################################################################
load("ZZ_1992_1994.Rdata")

#make caterpillar plot from MCMCvis package
##!!READ THIS!!! To save these plots you must got to the plot viewer,
...
##
MCMCplot(ZZ,params = c("alphasig","betasig2", "betaT2","betaT3","alphaphi",#"betaphiT",
                      "tauv"),ci = c(50, 90), guide_lines = TRUE,HPD = TRUE,
         xlab = 'Parameter estimate',sz_med = 1.1,sz_labels = 1.0,sz_ax = 1.1,
         #pos_tick=c(-6,-4,-2,0,2,4,6,8,10),
         sz_tick_txt=0.8,sz_ax_txt=1.1,
         labels = c("Scale int","Scale temp","Movement temp_max", "Movement temp_delta",
                    "Survival int","Precision")) 
...
############################################################
##########get lambda estimates for manuscript with 90% CIs
(meanlam <- mean(lam0,na.rm = TRUE))
(lCLlam <- quantile((lam0),probs= 0.10,na.rm = TRUE)) 
(UCLlam <- quantile((lam0),probs= 0.90,na.rm = TRUE))
############################################################
...
for (j in 1:42000){
  for(f in 1:23){
    fish[f] <-  prod(phi[j,f,1:19], na.rm=TRUE)
  }
...
cols <- c("4"="red1","5" = "blue4", "6"="red3","9" = "darkorchid1", "11" = "orange",
          "13" = "cyan2", "14" = "gold1","16" = "gold4", "18" = "purple2", "19" = "magenta1","20" = "blue1")
ggformat <- (
  ggplot(longmov, aes(x = week, y=99-avg.S, color=fish)) + #96.7-
    geom_point()+
    geom_line()+
    scale_colour_manual(values = cols)+
    geom_line(aes(y = TempB*6.5), color="black",linetype = "dashed")+
    geom_line(aes(y = TempE*6.5), color="black",linetype = "dashed")+
    scale_x_continuous(breaks = c(1,3,5,7,9,11,13,15,17),labels=c("1" = "Aug 19", 
                      "3" = "Sept 16", "5" = "Oct 14","7" = "Nov 11","9" = "Dec 9",
                      "11" = "Jan 6", "13" = "Feb 4","15" = "Mar 4","17" = "Apr 1"))+
    scale_y_continuous("Reach location (RKM)",limits = c(-20, 100),
...
ggsave(filename = "BT_MovementsPOSTLONGMOVOct302023c.png", plot=ggformat,
...
ggsave(filename = "BT_MovementsPOSTMIDMOVOct302023.png", plot=ggformat,
       device="png",height=5,width=6,units = "in",dpi=800)
#########################################################################
#############################################################################################
abmov<- df%>%
...
ggsave(filename = "BT_MovementsPOSTABMOVOct302023.png", plot=ggformat,
       device="png",height=5,width=6,units = "in",dpi=800)
